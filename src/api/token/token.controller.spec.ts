...
describe('TokenController', () => {
  ...
  describe('exchange', () => {
    it('should return new token', async () => {
      const result = await tokenController.exchange(mockToken);
      expect(result).toHaveProperty('token');
    });
    it('should return ForbiddenException with wrong token', async () => {
      try {
        await tokenController.exchange({ token: '123' });
      } catch (err) {
  ...
  describe('createServiceToken', () => {
    it('should return new internal token for service', async () => {
      const result = await tokenController.createServiceToken('test');
